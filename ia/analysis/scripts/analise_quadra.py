...
def analyze_directory(directory_path):
    ...
    # Custom sorting key function to handle numerical values in file names
    def sort_key(name):
        # Use regular expression to find numbers in the file name
        numbers = re.findall(r'\d+', name)
        # Return a tuple of the name and numerical part for sorting
        return (name, int(numbers[0]) if numbers else 0)

    # Get a sorted list of log files
    log_files = sorted(
        (f for f in os.listdir(directory_path) if f.endswith('.log')),
        key=sort_key
    )

    print(f'Found {len(log_files)} log files in {directory_path}')
    # Loop through each file in the sorted list of log files
